...
function TodoItem(props) {
  const detailLink = "/tasks/" + props.task.taskId;
  ...
  return (
    <tbody>
      <tr>
        ...
        <td> <Link to={detailLink} >{props.task.taskTitle}</Link>  </td> <td> {props.task.taskStatus} </td>
        <td> {props.task.taskDescription} </td>
        <td> {props.task.taskDueDate} </td> <td> Requestor </td>
        <td> Assignee </td> <td> {props.task.taskPriority} </td>
        <td>
          <Link to={editLink}>
            <FaRegEdit className="icons" onClick={props.task.handleEditClick} />
          </Link>
          <Link to={deleteLink}>
            <MdDelete
              className="icons"
